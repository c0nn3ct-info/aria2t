...
- 为完成的种子做种（到一个停止分享率，默认 1.0），并可按下载或作为已保存的全局默认值进行控制——停止分享率、做种时长、Tracker 列表。
- 友好的首次运行界面，一键从剪贴板添加链接；仍有下载在进行时退出会先提示确认。
- 多服务器切换并测量延迟；下载完成或失败时终端响铃并报出名字。
- 完整鼠标支持，无需双击：单击某个下载即可打开其详情，快捷键栏的每一个提示都可点击（暂停、删除、选择文件、连接服务器……）。每个对话框都带有统一的实心**取消**/确认按钮（任何破坏性操作的确认按钮为红色）——一切皆可用鼠标**和**键盘触及。滚轮滚动。
- **退出时不丢失任何东西**：已完成的下载在重启后仍留在列表中，未完成的传输会恢复，而你未作答就关闭的文件选择器会在下次启动时重新打开。
...
默认情况下，aria2t 在 `PATH` 中找到 `aria2c`，在空闲端口上用随机密钥启动一个私有守护进程，并托管其完整生命周期：完整的会话——已完成的下载和进行中的传输一并——在退出时保存、下次启动时恢复（已完成的会重新显示为已完成，而不会被重新下载），退出时干净地停止子进程（saveSession + shutdown RPC，仅在必要时升级为信号）。如果某次崩溃导致守护进程仍在运行，下次启动会先回收它，再启动一个全新的进程。无需配置，也不会留下残留进程。
...
| 鼠标 | 单击某行 → 详情 · 点击快捷键栏任意提示（暂停、文件、连接……） · 点击对话框的取消/确认按钮 · 点击 FILES 面板 → 选择器 · 滚轮滚动 · 无需双击 |
...
带 `managed` 的服务器是内置守护进程（端口和密钥在启动时决定）。其余为外部端点；`path` 可为反向代理后的守护进程覆盖 RPC 路径。`globalDown`/`globalUp` 是保存的总体速度上限（连接时重新应用到守护进程；省略或设为 `""` 表示不限速）。`seedRatio`/`seedTime` 是保存的全局做种默认值（同样在连接时重新应用，独立于调度器）。省略的字段保持默认值。
