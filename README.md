...
- Seeds finished torrents (to a stop ratio, by default 1.0) and controls it per download or as a saved global default — stop ratio, seed time, tracker list.
...
- Full mouse support with **no double-clicks**: a single click on a download opens its details, and every key-bar hint is clickable (pause, remove, select files, connect a server…). Every dialog carries consistent filled **Cancel** / confirm buttons (a red confirm for anything destructive) — everything is reachable by mouse *and* by keyboard. Wheel scrolls.
- **Nothing is lost on quit**: completed downloads stay in the list across restarts, unfinished transfers resume, and a file picker you closed without answering reopens on the next launch.
...
By default aria2t finds `aria2c` on your `PATH`, spawns a private daemon on a free port with a random secret, and manages its whole lifecycle: the full session — completed downloads and in-progress transfers alike — is saved on exit and restored on the next launch (completed ones reappear as finished, they are not re-downloaded), and the child is stopped cleanly (saveSession + shutdown RPC, escalating to signals only if needed) when you quit. If a crash ever leaves a daemon running, the next launch reaps it before starting a fresh one. Nothing to configure, nothing left running.
...
| Mouse | click a row → details · click any key-bar hint (pause, files, connect…) · click a dialog's Cancel/confirm button · click the FILES panel → picker · wheel scroll · no double-click needed |
