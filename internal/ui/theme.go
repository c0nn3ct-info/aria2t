...
type Styles struct {
	...
	// "fix" them by removing the background.
	BtnPrimary lipgloss.Style // Accent fill — primary commit (rightmost)
	BtnDanger  lipgloss.Style // Red fill — destructive commit
	BtnNeutral lipgloss.Style // muted fill — Cancel / secondary (leftmost)
...
func NewStyles(p Palette) Styles {
	...
	return Styles{
		...
		BtnPrimary: lipgloss.NewStyle().Background(p.Accent).Foreground(p.Bg).Bold(true).Padding(0, 2),
		BtnDanger:  lipgloss.NewStyle().Background(p.Red).Foreground(p.Bg).Bold(true).Padding(0, 2),
		BtnNeutral: lipgloss.NewStyle().Background(p.Sel).Foreground(p.Fg).Bold(true).Padding(0, 2),
