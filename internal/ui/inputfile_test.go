...
func TestAddSubmitInputFile(t *testing.T) {
	...
	if _, cmd := m.submit(); cmd == nil || !strings.Contains(a.status, "no valid links") {
	...
	if _, cmd := m.submit(); cmd == nil || !strings.Contains(a.status, "binary file") {
	...
	if a.overlay != overlayNone {
		t.Fatalf("submit must close, overlay=%d", a.overlay)
	}
	drain(t, a, cmd)
