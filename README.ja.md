...
- 完了した torrent をシードし（停止レシオまで、デフォルトは 1.0）、ダウンロードごとに、または保存された全体のデフォルトとして制御します — 停止レシオ、シード時間、トラッカー一覧。ピアとファイル選択も表示。
- 親切な初回起動画面 — クリップボードのリンクをワンタップで追加。ダウンロードが実行中のまま終了しようとすると警告します。
- レイテンシ計測付きでサーバーを切り替え。完了・失敗時にはターミナルベルを鳴らしてダウンロード名を通知。
- ダブルクリック不要の完全なマウス対応: ダウンロードをシングルクリックすると詳細が開き、キーバーのヒントはすべてクリック可能（一時停止、削除、ファイル選択、サーバーへ接続…）。すべてのダイアログには一貫した塗りつぶしの **キャンセル**／確定ボタンがあり（破壊的な操作には赤い確定ボタン）— すべてマウスでも *キーボードでも* 操作できます。ホイールでスクロール。
- **終了時に失われるものはありません**: 完了したダウンロードは再起動をまたいで一覧に残り、未完了の転送は再開され、答えずに閉じたファイルピッカーは次回起動時に再び開きます。
...
デフォルトでは、aria2t は `PATH` から `aria2c` を見つけ、空いているポートにランダムな secret で専用デーモンを起動し、そのライフサイクル全体を管理します。セッション全体 — 完了したダウンロードも進行中の転送も同様に — は終了時に保存され次回起動時に復元され（完了したものは完了済みとして再び表示され、再ダウンロードされることはありません）、子プロセスは終了時にクリーンに停止します（saveSession + shutdown RPC、必要な場合のみシグナルへエスカレーション）。クラッシュによってデーモンが起動したまま残ってしまった場合でも、次回起動時にそれを回収してから新しいデーモンを起動します。設定は不要で、後に残るものもありません。
...
| マウス | 行をクリック → 詳細 · キーバーのヒントをクリック（一時停止、ファイル、接続…） · ダイアログのキャンセル／確定ボタンをクリック · FILES パネルをクリック → ピッカー · ホイールでスクロール · ダブルクリック不要 |
...
`managed` のサーバーは内蔵デーモンです（ポートと secret は起動時に選ばれます）。それ以外は外部エンドポイントで、`path` はリバースプロキシ配下のデーモン向けに RPC パスを上書きします。`globalDown`／`globalUp` は保存された全体の速度上限です（接続時にデーモンへ再適用されます。無制限にするには省略するか `""` を指定します）。`seedRatio`／`seedTime` は保存された全体のシードのデフォルトです（こちらも接続時に再適用され、スケジューラとは独立しています）。省略した項目はデフォルト値を保ちます。
