...
// a story that was never written: ten landing bands and the page itself shipped
// without one, and the Storybook built green the whole time, because nothing
// asked for them.
//
// The rule is the convention the rest of the tree already follows. A `.tsx`
// under `src/components/` or `src/pages/` that is neither a test nor a story
// has a sibling `*.stories.tsx`. Hooks and pure modules are `.ts` and render
// nothing, so they are not in scope.
//
