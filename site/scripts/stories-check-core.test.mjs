// The missing-story gate. The walk and the report are pure given a file system,
// so both run against a fake one; `main` is checked on the real tree, which is
// the assertion that matters — every component and page in this repo has a
// story right now.
