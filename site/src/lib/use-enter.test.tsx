...
const observers: { cb: IoCallback; targets: Element[]; unobserved: Element[]; dead: boolean }[] = [];
...
function Page({ dir = 'ltr' }: { dir?: string }) {
  useSectionEntrance();
  return (
    <>
      ...
      <section data-testid="unmarked" data-dir={dir} />
    </>
  );
}
...
beforeEach(() => {
  ...
  // setup's matchMedia answers every query with the system-dark flag, which is
  // false, so `(prefers-reduced-motion: no-preference)` reads false too.
...
describe('useSectionEntrance', () => {
  ...
  it('plays each marked element once the band comes up, in reading order', () => {
    ...
    const io = observers[0] as unknown as { fire: (e: Element[], v?: boolean) => void; unobserved: Element[] };
  ...
  it('lets go of the observer when the page goes', () => {
    ...
    unmount();
    expect((observers[0] as unknown as { dead: boolean }).dead).toBe(true);
  });
});
