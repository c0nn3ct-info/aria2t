...
//
// Ported from ../c0nn3ct, whose own comment carries the measurement that
// settled the approach: a fixed-duration entrance has no relationship to the
// scroll, so at an ordinary wheel speed every element began well below the fold
// and finished above the top of the screen, and whatever the reader caught was
// an arbitrary phase of it that then snapped to done. A `view()` timeline makes
// progress a function of position instead, so the element is always in the
// state its place on screen calls for.
//
 ...
 * keyframe rule and a keyframe object cannot share one definition. Transform
