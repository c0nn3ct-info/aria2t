...
- Раздаёт завершённые торренты (до stop-ratio, по умолчанию 1.0) и управляет этим per-download или как сохранённым глобальным значением по умолчанию — stop-ratio, время сидирования, список трекеров.
...
- Полная поддержка мыши без двойных кликов: одиночный клик по загрузке открывает её детали, а каждая подсказка в строке клавиш кликабельна (пауза, удалить, выбор файлов, подключить сервер…). В каждом диалоге — единообразные кнопки с заливкой **Cancel** / подтверждение (красное подтверждение для всего разрушительного) — всё доступно и мышью, и с клавиатуры. Колесо листает.
- **Ничего не теряется при выходе**: завершённые загрузки остаются в списке между перезапусками, незавершённые передачи возобновляются, а окно выбора файлов, закрытое без ответа, открывается снова при следующем запуске.
...
По умолчанию aria2t находит `aria2c` в `PATH`, поднимает приватный демон на свободном порту со случайным секретом и полностью сопровождает его жизненный цикл: вся сессия — и завершённые загрузки, и текущие передачи — сохраняется при выходе и восстанавливается при следующем запуске (завершённые снова появляются как готовые, они не скачиваются заново), а дочерний процесс останавливается аккуратно (RPC saveSession + shutdown, сигналы — только если не помогло). Если из-за сбоя демон остаётся работать, следующий запуск завершает его, прежде чем поднять новый. Ничего настраивать не нужно, ничего не остаётся висеть.
...
| Мышь | клик по строке → детали · клик по любой подсказке в строке клавиш (пауза, файлы, подключить…) · клик по кнопке Cancel/подтверждения в диалоге · клик по панели FILES → выбор файлов · колесо — прокрутка · двойной клик не нужен |
...
Сервер с `managed` — встроенный демон (порт и секрет выбираются при запуске). Остальные — внешние; `path` переопределяет путь RPC для демонов за reverse-proxy. `globalDown`/`globalUp` — сохранённые общие лимиты скорости (заново применяются к демону при подключении; пропустите или `""` — без лимита). `seedRatio`/`seedTime` — сохранённые глобальные значения раздачи по умолчанию (тоже заново применяются при подключении, независимо от планировщика). Пропущенные поля получают значения по умолчанию.
